...
class PaparazziController extends Controller
{
    ...
    */
    ...
    {
        $post = Post::orderby('priority', 'ASC')->oldest()->first();
        // look for a new post (priority == 0)
        if ($post->priority == 0) {
            $maxPriorityPost = Post::orderby('priority', 'DESC')->first();
            if ($maxPriorityPost != null) {
                if ($maxPriorityPost->priority == 0){
                    $post->increment('priority');
                } else {
                    $post->update(['priority' => $maxPriorityPost->priority]);
                }
            }
        }  else {
            $post->increment('priority');
